...
public class BaseDao<T, S> {
    ...
    protected int insertForId(String sql, T t, String pkField) {
        ...
        int rc = namedParameterJdbcTemplate.update(sql, new BeanPropertySqlParameterSource(t), keyHolder,
                new String[]{pkField});
        if (rc > 0) {
    ...
    protected List<T> list(String sql, S s) {
        List<T> list = namedParameterJdbcTemplate.query(sql, new BeanPropertySqlParameterSource(s),
     ...
     * @return
     */
    protected List<ComboboxVO> listCombobox(String sql, S s) {
        List<ComboboxVO> list = namedParameterJdbcTemplate.query(sql, new BeanPropertySqlParameterSource(s),
                new BeanPropertyRowMapper<>(ComboboxVO.class));
        return list;
    }
     ...
     * @return
     */
    protected Integer sumInt(String sql, Object... objects) {
        return jdbcTemplate.queryForObject(sql, objects, Integer.class);
    }
}
